...
func main() {
	...
	window.Call("addEventListener", "click", mouseEventHandler)
...
func updateGame() {
	...
	laserCtx.Call("arc", gs.laserX, gs.laserY, gs.laserSize, 0, math.Pi*2)
	laserCtx.Call("fill")
...
func updatePlayer(event js.Value) {
	...
	if isLaserCaught(mouseX, mouseY) {
		go playSound()
	}
}
...
func isLaserCaught(mouseX, mouseY float64) bool {
